...
interface PageHeaderProps {
  ...
  subtitle?: ReactNode;
  actions?: ReactNode;
  // When set, a "How to" link to /docs/<docsGuide> appears in the header.
  docsGuide?: string;
}
...
export default function PageHeader({ title, subtitle, actions, docsGuide }: PageHeaderProps) {
  return (
    <div className={`flex ${subtitle ? "items-start" : "items-center"} justify-between gap-3`}>
      ...
      {(actions || docsGuide) && (
        <div className="flex shrink-0 items-center gap-2">
          {docsGuide && <DocsLink guide={docsGuide} />}
          {actions}
        </div>
      )}
    </div>
