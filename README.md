...
On an Ubuntu server, `deploy/deploy.sh` is an interactive installer: it asks
Docker or native, and for Docker which optional add-ons to include (one-click
updater, local AI), then brings the stack up. `deploy/portwiz.service` runs it as
a systemd service. See
[docs/installation.md](docs/installation.md#installing-on-ubuntu-interactive).
